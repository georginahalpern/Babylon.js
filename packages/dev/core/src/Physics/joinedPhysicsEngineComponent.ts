...
declare module "../scene" {
    ...
    export interface Scene {
        ...
        /**
         * When called on scene, this function will generate physics shapes for any meshes which have checkCollisions set to true, and then use those physics shapes for collision detection whenever moveWithCollisions is called (overriding the existing moveWithCollisions logic with physics v2 logic)
         * NOTE: this function must be called again if the scene changes a mesh's isEnabled property or its checkCollisions property
         * @param hk HavokPlugin
         ...
         * @returns a boolean indicating if the physics engine was initialized
         */
        enablePhysicsForMoveWithCollisions(hk: HavokPlugin, meshes?: AbstractMesh[]): boolean;
...
Scene.prototype.enablePhysicsForMoveWithCollisions = function (plugin: HavokPlugin, meshes?: AbstractMesh[]): boolean {
    ...
    const successfullyEnabled = this._physicsEngine?.enablePhysicsForMoveWithCollisions(meshes || this.meshes);
    return !!successfullyEnabled;
};
