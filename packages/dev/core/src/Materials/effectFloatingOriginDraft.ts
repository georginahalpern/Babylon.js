...
export const WorldOffset = (world: Matrix, offset: IVector3Like) => TmpVectors.Matrix[4].copyFrom(world).addTranslationFromFloats(-offset.x, -offset.y, -offset.z);
export const ViewOffset = (view: Matrix) => TmpVectors.Matrix[4].copyFrom(view).setTranslationFromFloats(0, 0, 0);
export const WorldViewOffset = (worldView: Matrix, offset: IVector3Like) => TmpVectors.Matrix[4].copyFrom(worldView).addTranslationFromFloats(-offset.x, -offset.y, -offset.z);
export const ViewProjectionOffset = (view: Matrix, projection: Matrix) => ViewOffset(view).multiplyToRef(projection, TmpVectors.Matrix[5]);
...
export const WorldOffsetS = (world: Matrix, scene: Scene) =>
    TmpVectors.Matrix[4].copyFrom(world).addTranslationFromFloats(-scene.floatingOriginOffset.x, -scene.floatingOriginOffset.y, -scene.floatingOriginOffset.z);
export const ViewOffsetS = (scene: Scene) => TmpVectors.Matrix[4].copyFrom(scene.getViewMatrix()).setTranslationFromFloats(0, 0, 0);
export const WorldViewOffsetS = (scene: Scene) =>
    TmpVectors.Matrix[4].copyFrom(scene.getTransformMatrix()).addTranslationFromFloats(-scene.floatingOriginOffset.x, -scene.floatingOriginOffset.y, -scene.floatingOriginOffset.z);
export const ViewProjectionOffsetS = (scene: Scene) => ViewOffset(scene.getViewMatrix()).multiplyToRef(scene.getProjectionMatrix(), TmpVectors.Matrix[5]);
export const WorldViewProjectionOffsetS = (scene: Scene) => WorldViewOffsetS(scene).multiplyToRef(scene.getProjectionMatrix(), TmpVectors.Matrix[5]);
