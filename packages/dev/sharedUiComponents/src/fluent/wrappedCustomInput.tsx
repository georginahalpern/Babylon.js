...
// Generic HOC for any component with onChange/onKeyDown
type WithStopPropagationProps = {
    onChange?: (...args: any[]) => void;
    onKeyDown?: (...args: any[]) => void;
};

function withStopPropagationOnChange<P extends WithStopPropagationProps>(Component: React.ComponentType<P>) {
    return (props: P) => {
        const handleChange = (...args: any[]) => {
            if (args[0]?.stopPropagation) args[0].stopPropagation();
            props.onChange?.(...args);
        };
...
const CustomText: React.FC<TextareaProps> = (props) => {
    const styles = useGlobalStyles();
...
//     const styles = useGlobalStyles();
//     return <Input {...props} className={styles.floatLineInput} onChange={props.onChange} onKeyDown={props.onKeyDown} />;
// };
