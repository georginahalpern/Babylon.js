...
import { MakePropertyHook, useProperty } from "./compoundPropertyHooks";
...
> & {
    ...
    customChange?: (value: TargetT[PropertyKeyT]) => void;
};
...
function BoundPropertyCoreImpl<TargetT extends object, PropertyKeyT extends keyof TargetT, ComponentT extends ComponentType<any>>(
    ...
    const SpecificComponent = useMemo(() => {
        return (props: ComponentProps<ComponentT>) => {
            // eslint-disable-next-line @typescript-eslint/naming-convention
            const { target, propertyKey, convertTo, convertFrom, customChange, component: Component, ...rest } = props;
            ...
            const propsToSend = {
                ...
                onChange: (val: TargetT[PropertyKeyT]) => {
                    ...
                    customChange && customChange(newValue);
                },
            };
